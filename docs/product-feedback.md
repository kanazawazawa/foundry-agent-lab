# Foundry Portal — プロダクトフィードバック（メモ）

※ いずれの項目もまだ正式にフィードバック報告していない。内部メモとして記録。
...
## 4. Azure Speech の [laughter] タグ使用後に言語スキルが無効化される

**メモ日**: 2026-04-15  
**ステータス**: 未報告（メモ）

**対象**: Azure Speech Service（読み上げ / Text-to-Speech）

**現象**:
- SSML 等で `[laughter]` タグを使用した直後、読み上げの言語スキル（発音・イントネーション等）が正しく機能しなくなる
- `[laughter]` タグ挿入前は正常に動作している
...
## 5. HD 音声（HD Voice）を日本リージョンでも提供してほしい

**メモ日**: 2026-04-16  
**ステータス**: 未報告（メモ）

**対象**: Azure Speech Service — HD 音声（Text-to-Speech HD voices）

**要望**:
- HD 音声は日本語（ja-JP）の音声モデルが存在するにもかかわらず、日本リージョン（Japan East / Japan West）ではデプロイできない
- 日本リージョンでも HD 音声を利用可能にしてほしい
